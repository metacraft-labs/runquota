## runquota/docs/book-isonim -- client JS mount entry.
##
## The compiled `nim js` bundle every book page loads (via
## `bookDocsConfig().appScriptHref` -> `/assets/app.js`, injected in `<head>`
## by the framework's document-head renderer). It is strict PROGRESSIVE
## ENHANCEMENT: the SSR HTML is already a complete, navigable page (plain `<a>`
## sidebar links, default-expanded sections), and this bundle only ADDS live
## behaviour on top of the DOM the server already rendered --
##
##   * the theme toggle (`#docs-theme-toggle`) flips + persists the theme,
##     sharing the exact `theme_vm` storage key/precedence the `<head>`
##     no-flash bootstrap uses;
##   * the search box (`#docs-search-input`) filters this site's own
