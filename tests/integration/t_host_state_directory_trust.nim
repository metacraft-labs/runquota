...
## NOT RESTRICTED ON WINDOWS, AND NOT PASSING THERE. The daemon's state-
## directory trust check has no Windows arm -- it answers "trusted" without
## looking -- and a POSIX mode is not the Windows form of the invariant (a
## DACL is). That is an open defect:
## `reprobuild-specs/issues/2026-09-24-runquota-windows-host-state-directory-trust-unchecked.md`.
## Until it is fixed every case FAILS on Windows, naming it, rather than
## failing to compile on the POSIX fixtures below.
...
when not defined(posix):
  ...
  suite "host_state_directory_trust":
    for name in [
        "a state directory this daemon owns and nobody else can write is ACCEPTED",
        "a WORLD-WRITABLE state directory is REFUSED, naming path and mode",
        "the refusal is the SAME on a second start",
        "a state directory owned by ANOTHER uid is REFUSED as an ownership problem",
        "a directory that is not there yet is reported by the PROVISIONING refusal, not this one"]:
      test name:
        checkpoint("host-state directory trust is not verified on Windows " &
          "(reprobuild-specs/issues/2026-09-24-runquota-windows-host-state-" &
          "directory-trust-unchecked.md)")
        fail()
else:
