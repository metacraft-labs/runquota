...
## POSIX ONLY, BY DESIGN. Everything asserted here is a property of the
## rendezvous DIRECTORY -- its group, its mode, the socket's mode, and the
## single-user degradation when the group does not resolve. On Windows the
## endpoint is a named pipe and there is no such directory:
## `docs/database.md` §"Provisioning the host-wide state directory and the
## rendezvous" lists the Windows rendezvous as "— (named pipes)" with no
## owner, group or mode, and `runquota_ipc.endpointDirectoryTrust` answers
## `trustOk` for a named pipe because "there is no directory to own and no
## mode to widen". So on Windows every case is reported skipped.
...
when not defined(posix):
  ...
  suite "rendezvous_degradation":
    for name in [
        "a resolvable group gives a SHARED, group-gated endpoint and says nothing",
        "an UNRESOLVABLE group degrades to owner-only, and SAYS so",
        "the two daemons differ in every observable the ruling is about"]:
      test name:
        skip()
else:
