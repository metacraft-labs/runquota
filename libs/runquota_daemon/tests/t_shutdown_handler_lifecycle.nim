...
##
## POSIX ONLY, BY DESIGN. Everything this file drives -- the SIGTERM handler,
## the self-pipe it writes one byte into, and the waker that dials an AF_UNIX
## socket -- is the `when defined(posix)` arm of `runquota_daemon`'s shutdown
## machinery. There is no SIGTERM on Windows; the section headed "WINDOWS HAS
## NO SIGTERM, AND THAT IS NOT THE SAME AS HAVING NO STOP" in
## `runquota_daemon` records the Windows arm: a stop from the Service Control
## Manager, observed by a watcher thread that opens the daemon's own named
## pipe. Neither hazard pinned here has a Windows form -- a pipe `CreateFileW`
## against a busy pipe fails with ERROR_PIPE_BUSY rather than waiting, and
## there is no handler writing into a descriptor -- so on Windows every case
## is reported skipped rather than compiled against symbols that do not
## exist there.
...
when not defined(posix):
  ...
  suite "shutdown handler lifecycle":
    for name in [
        "the waker's dial comes back even when the backlog is full",
        "disarming leaves the handler's descriptor valid rather than reusable",
        "the handler's write cannot block, however full the pipe is",
        "a wake token left by one disarm does not disarm the next waker",
        "a byte from a stranded handler does not disarm the next waker"]:
      test name:
        skip()
else:
